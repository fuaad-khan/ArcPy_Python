...
#         and the distance to the lake shoreline*). Thes new fields are from the
#         closest city to the shorline from each lake.
...
uFields = ["SHAPE@", "City_Name", "City_Cntry", "Admin_Name", "City_Pop", "City_X", "City_Y", "Centr_Dist"]
field_update = arcpy.da.UpdateCursor(newLake, uFields)

#Loop through the update cursor
for field in field_update:
    #Create two empty lists to hold the vertices of the lakes
    listXvertices = []
    listYvertices = []
    #Loop through the SHAPE@ geometry for the lake shapefile and add all the x,y vertices to the list
    for f in field[0]:
        for point in f:
            if point:
                listXvertices.append(point.X)
                listYvertices.append(point.Y)
                
    #Initialize list of the closest city
    listClosest_city = []
    
    #Loop through the range of cities appended to the empty lists
    for city in range(0, len(listCity_Name)):
        listDistance_vertices = []
        #Assign Longitude/Latitude city values from out appended list
        longCity = listCity_X[city]
        latCity = listCity_Y[city]

        #Loop through the range of the list of vertices and assign the longitude and latitude
        for r in range(len(listXvertices)):
            longLake = listXvertices[r]
            latLake = listYvertices[r]

            #Calculate the distance from the shoreline vertices of the lake to the cities
            distance = math.sqrt(((longLake - longCity)**2) + ((latLake - latCity)**2))
            #Add these calcualtions to our list that stores distance
            listDistance_vertices.append(distance)
        listClosest_city.append(min(listDistance_vertices))
  
    #Use a function that allows us to find the city with the least amount of distance i.e. closest city
    closestCity = min(listClosest_city)

    #Create an index and assign the proper index to the update field
    closestCityIndex = listClosest_city.index(closestCity) 
